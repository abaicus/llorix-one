...
function llorix_one_scripts() {
	...
	wp_enqueue_style( 'llorix-one-font', '//fonts.googleapis.com/css?family=Cabin:400,600|Open+Sans:400,300,600');

	wp_enqueue_style( 'llorix-one-bootstrap-style', llorix_one_get_file( '/css/bootstrap.min.css'),array(), '3.3.1');

	wp_enqueue_style( 'llorix-one-style', get_stylesheet_uri(), array('llorix-one-bootstrap-style'),'1.0.0');
	
	wp_enqueue_script( 'llorix-one-bootstrap', llorix_one_get_file('/js/bootstrap.min.js'), array(), '3.3.5', true );
		
	wp_enqueue_script( 'llorix-one-custom-all', llorix_one_get_file('/js/custom.all.js'), array('jquery'), '2.0.2', true );
	
	wp_localize_script( 'llorix-one-custom-all', 'screenReaderText', array(
...
function llorix_one_add_id(){
	...
	if( isset($migrate) && $migrate == false ) {
		...
		/*Logo*/
		$llorix_one_logos = get_theme_mod('parallax_one_logos_content', json_encode(
			array( 
		...
		if(!empty($llorix_one_logos)){
			...
			set_theme_mod( 'parallax_one_logos_content', $llorix_one_logos );
		}
		
		
		/*Services*/
		$llorix_one_services = get_theme_mod('parallax_one_services_content', json_encode(
							array(
		...
		if(!empty($llorix_one_services)){
			...
			set_theme_mod( 'parallax_one_services_content', $llorix_one_services );
		}
		
		/*Team*/
		$llorix_one_team = get_theme_mod('parallax_one_team_content', json_encode(
							array(
		...
		if(!empty($llorix_one_team)){
			...
			set_theme_mod( 'parallax_one_team_content', $llorix_one_team );
		}
		
		/*Testimonials*/
		$llorix_one_testimonials = get_theme_mod('parallax_one_testimonials_content', json_encode(
							array(
		...
		if(!empty($llorix_one_testimonials)){
			...
			set_theme_mod( 'parallax_one_testimonials_content', $llorix_one_testimonials );
		}
		
		/*Contact Info*/
		$llorix_one_contact_info = get_theme_mod('parallax_one_contact_info_content', json_encode(
			array( 
		...
		if(!empty($llorix_one_contact_info)){
			...
			set_theme_mod( 'parallax_one_contact_info_content', $llorix_one_contact_info );
		}
		
		/*Social Icons*/
		$llorix_one_social_icons = get_theme_mod('parallax_one_social_icons', json_encode(
			array(
		...
		if(!empty($llorix_one_social_icons)){
			...
			set_theme_mod( 'parallax_one_social_icons', $llorix_one_social_icons );
		}
...
$llorix_one_notification = new TAV_Remote_Notification_Client( 49, 'a0973bf1bd1fe265', 'https://themeisle.com?post_type=notification' );
...
function llorix_one_php_style() {
	...
	$llorix_one_title_color = get_theme_mod('parallax_one_title_color');
	if(!empty($llorix_one_title_color)){
		echo '.dark-text { color: '. $llorix_one_title_color .' }';
	}
	$llorix_one_text_color = get_theme_mod('parallax_one_text_color');
...
function llorix_one_prevent_wporg_update( $r, $url ) {
...
add_filter( 'http_request_args', 'llorix_one_prevent_wporg_update', 5, 2 );
...
if(function_exists('icl_unregister_string') && function_exists('icl_register_string')){
	...
	/*Services*/
	$llorix_one_services_pl = get_theme_mod('parallax_one_services_content');
	...
	/*Team*/
	$llorix_one_team_pl = get_theme_mod('parallax_one_team_content');
	...
	/*Testimonials*/
	$llorix_one_testimonials_pl = get_theme_mod('parallax_one_testimonials_content');
	...
	/*Contact*/
	$llorix_one_contact_pl = get_theme_mod('parallax_one_contact_info_content');
